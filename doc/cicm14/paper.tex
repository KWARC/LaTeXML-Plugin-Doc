...
Then we use an XSLT stylesheet to generate document.xml from the .ltmxl file. The same goes for relations.xml .The other supporting files such as images are placed into the correct file structure the postprocessor. As the penultimate step some static files, that don't change depending on the input document, are also placed into the correct directories. Finally the document is zipped to create the docx/odt file. \\ 
...
In Conclusion we use \latexml and XSLT to transform {\LaTeX} files to Word/Office files
semantically in an easy to use process.  \ednote{In Conclusion, easy to use}

The \latexml Word Processing plugin is public domain and is available from GitHub
