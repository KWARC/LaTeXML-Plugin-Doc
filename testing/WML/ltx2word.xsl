...
<pic:nvPicPr>
                    <pic:cNvPr id="0" name="don't know what I should write here"/><!-- not deletable, however I can change both the name and id properties. --> 
                    <pic:cNvPicPr/> <!-- not removable either -->
                  </pic:nvPicPr>
                  <pic:blipFill>
                  <xsl:variable name="foo" select="generate-id(.)"/>
                    <a:blip r:embed="{$foo}"/> <!-- This encodes what picture to actually take, by the id specified in document.xml.rels " -->
                    <picturus id="{$foo}" reference="{concat('media/',@candidates)}"/>
                    <a:stretch>
                      <a:fillRect/> <!-- This stuff tells Word to stretch the image to the frame size -->
                    </a:stretch>
                  </pic:blipFill>
                  <pic:spPr>
                    <a:xfrm>
                      <a:off x="0" y="0"/> <!-- offset. Is deletable if there is no offset. --> 
                      <a:ext cx="{9525*@imagewidth}" cy="{9525*@imageheight}"/> <!-- I think this is just the size of the frame or something. Is deletable and values are changable, however there is no real picture if the values differ relevantly from the size of the frame given beforehand. Don't mess around with it I guess --> 
                    </a:xfrm>
...
<xsl:template match="ltx:graphics"> <!-- These are probably not inline, but I am treating them as inline for this period. --> 
    <w:p>
      <w:r>
      	<w:rPr/>
  	<w:drawing>
